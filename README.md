...
| 4 | Word 出力 | 完了（実機確認済み） |
| 5 | 共有と PWA 化 | 実装済み（実機確認待ち） |

現在の版: **v5.0.0**（蔵書画面の右上に表示される）
...
`navigator.canShare({files:[file]})` で判定し、可なら共有シート、不可ならダウンロード。
Word 目録と JSON バックアップの両方で使う。

**`navigator.share` は必ずタップのハンドラ内から、`await` を挟まずに呼ぶ。**
iOS はユーザー操作から間が空くと拒否するため、
「ファイルを作る」と「共有する」を別のタップに分けてある。

1. 「Word 目録を出力」を押す → 範囲を選ぶ → **Blob を作り終える**
2. 出来上がったファイル名とサイズを出したダイアログを表示
3. 「共有…」を押した**そのハンドラの中で** `navigator.share` を呼ぶ

実際に `navigator.userActivation.isActive === true` の状態で呼ばれていることを
本物のタップで確認済み。

共有シートをユーザーが閉じただけ（`AbortError`）はエラー扱いしない。

### PWA

| ファイル | 中身 |
|---|---|
| `manifest.json` | `display: standalone` / `orientation: portrait` / テーマ色 `#0f1115` |
| `icons/` | 192・512・512(maskable)・apple-touch-icon 180 の PNG。`make-icons.py` で生成 |
...
| 対象 | 方針 | 理由 |
|---|---|---|
| `index.html` ほかアプリ本体 | **ネットワーク優先**、失敗したらキャッシュ | キャッシュ優先にすると更新が端末に届かなくなる |
| `cdn.jsdelivr.net`（zxing-wasm / docx） | **キャッシュ優先** | URL にバージョンが入っていて中身が変わらない。オフラインでも読取エンジンが動く |
| `api.openbd.jp` / `googleapis.com` / `books.google.com` | **一切キャッシュしない** | 古い応答を返すと登録内容が狂う |

### 更新の届け方

新しい版を見つけても**勝手にリロードしない**。スキャン中に画面が飛ぶのを防ぐため、
画面下に「新しい版があります ［更新する］［あとで］」を出し、
押されたときだけ `skipWaiting` → `controllerchange` で一度だけ読み直す。

アプリに復帰するたび（`visibilitychange`）に更新を確認する。
いま動いている版は蔵書画面の右上に出るので、端末に届いたかどうかを目で確かめられる。

**版を上げるときは `index.html` の `APP_VERSION` と `sw.js` の `VERSION` を揃えて変えること。**
